...
plot4<-function()
{
  G<-prepare()
  par(mfrow=c(2,2))
  plot(G$Date,G$Global_active_power,type="l",ylab="Global Active Power",xlab="")
  plot(G$Date,G$Voltage,type="l",ylab="Voltage",xlab="")
  ...
  legend("topright", inset=.05, cex = 0.5, title=, c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), border = "white",horiz=FALSE, lty=c(1,1),bty='n', lwd=c(3,3),col=c(8,2,4), bg="white")
  plot(G$Date,G$Global_reactive_power,type="l",ylab="Global_reactive_power",xlab="")
}